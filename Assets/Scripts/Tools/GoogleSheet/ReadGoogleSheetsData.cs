...
{
	public class ReadGoogleSheetsData
	{
		...
		public static void FillData<T>( string id , string gridId , Action<List<T>> calBack ) where T : new()
		{
			...
			GetTable(id , gridId , callBack: arr =>
			{
				var type = typeof(T);
							...
							for( int j = 0; j < lst.Count; j++ )
							{
								...
								if( !string.IsNullOrEmpty(value) )
								{
									...
									else
									{
										...
										{
											var obj = Int32.Parse(value);
											property.SetValue(lst[j] , obj);
										}
										else if( x.Name == "Single" )
										{
											var obj = float.Parse(value , CultureInfo.InvariantCulture);
											property.SetValue(lst[j] , obj);
										}
										else
										{
											Debug.Log("value:" + value + " - " + property.Name + " - " + property.FieldType);
